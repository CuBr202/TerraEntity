...
public record NPCTrades<T extends ITrade>(List<T> trades) {
    ...
    public static final Codec<NPCTrades> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ITrade.TYPED_CODEC.codec().listOf().fieldOf("trades").forGetter(NPCTrades::trades)
    ).apply(instance, NPCTrades::new));
    public static FriendlyByteBuf.Writer<NPCTrades> WRITER = AdapterUtils.CodecWriter(CODEC);
    public static FriendlyByteBuf.Reader<NPCTrades> READER = AdapterUtils.CodecReader(CODEC);
...
//    public static final StreamCodec<ByteBuf, NPCTrades> STREAM_CODEC = ByteBufCodecs.fromCodec(CODEC);
    ...
    public static FriendlyByteBuf.Writer<Map<ResourceLocation,NPCTrades>> MAP_WRITER = AdapterUtils.CodecWriter(MAP_CODEC);
    public static FriendlyByteBuf.Reader<Map<ResourceLocation,NPCTrades>> MAP_READER = AdapterUtils.CodecReader(MAP_CODEC);
