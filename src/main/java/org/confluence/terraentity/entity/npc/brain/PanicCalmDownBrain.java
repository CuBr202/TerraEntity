 ...
 * 恐慌行为清除
 */
public class PanicCalmDownBrain extends Behavior<LivingEntity> {
    ...
    public static boolean hasHostile(LivingEntity living) {
        return living.getBrain().hasMemoryValue(MemoryModuleType.NEAREST_HOSTILE);
    }

    public static boolean isHurt(LivingEntity living) {
        return living.getBrain().hasMemoryValue(MemoryModuleType.HURT_BY);
    }

    public static boolean isHurtByEntity(LivingEntity living) {
