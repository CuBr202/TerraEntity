...
public class WorldChunksManager {
    private static final Map<ServerLevel, WorldData> WORLD_DATA = new HashMap<>();
    static final int cartLoadDuration = 30;
    /**
     * Returns the persistent file for a given world.
     * @param world target world
     * @return the world's persistent file
     */
    public static Optional<File> getPersistentFileForWorld(ServerLevel world) {
        Path worldRoot = world.getServer().getWorldPath(LevelResource.ROOT);
        String worldId = world.dimension().location().getPath();
     ...
     */
    public static int loadPersistentChunksForWorld(ServerLevel world) {
        int loadedCount = 0;
     ...
     * @return loaded chunks
     */
    public static int savePersistentChunksForWorld(ServerLevel world) {
    ...
    public static void forceLoadChunk(ServerLevel world, ChunkPos pos) {
        ...
        long expiryTick = currentTick + cartLoadDuration * 20;
        if (worldData.addForcedChunk(new ChunkData(world.dimension().location().toString(), pos, expiryTick, currentTick)) == null) {
