...
public class WallOfFleshTranslucent {
    ...
    public static Map<WallOfFlesh, tuple> entityMap = new HashMap<>();
    ...
    public static void render(RenderLevelStageEvent event){
        ...
        List<WallOfFlesh> shouldBeRemoved = new ArrayList<>();
        ...
        TextureTarget in;
        int c = 0;
        for(WallOfFlesh wall : entityMap.keySet()){
            if(wall!= null){
                // 对每个BOSS本体虚影渲染
                c++;
                ...
                in = (c & 1) == 0? temp2: temp;
                out.bindWrite(true);
                tuple tuple = entityMap.get(wall);
                float alpha = Math.clamp(wall.getFadeProgress(), 0, 1);
                if(alpha < 0.99f){
...
//                    });
                    ...
                    float p = Math.clamp(wall.getFadeProgress(), 0, 1);
                    ((IShaderInstance)ModRenderTypes.Shaders.dissolveBlitLagerShader).getTerra_entity$Progress().set(1-p);
                    ((IShaderInstance)ModRenderTypes.Shaders.dissolveBlitLagerShader).getTerra_entity$Distance().set(Minecraft.getInstance().player.distanceTo(wall));
                    ShaderUtil.blitScreen(ModRenderTypes.Shaders.dissolveBlitLagerShader, shader->{
                        shader.COLOR_MODULATOR.set(0.5f, 0f, 1f, alpha);
                        shader.setSampler("Sampler0", finalIn);
                        shader.setSampler("Sampler1", target);
                        shader.setSampler("Sampler2",Minecraft.getInstance().getTextureManager().getTexture(TerraEntity.space("textures/gui/noise.png")) );
                    });
                }else{
                    shouldBeRemoved.add(wall);
                }
            }else{
                shouldBeRemoved.add(wall);
            }
        }
        for(WallOfFlesh brain : shouldBeRemoved){
