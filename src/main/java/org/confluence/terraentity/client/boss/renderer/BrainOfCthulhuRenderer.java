...
public class BrainOfCthulhuRenderer extends GeoNormalRenderer<BrainOfCthulhu> {
    ...
    @Override
    public void preRender(PoseStack poseStack, BrainOfCthulhu animatable, BakedGeoModel model, @Nullable MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
    ...
    public void render(BrainOfCthulhu entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        ...
        float alpha = org.joml.Math.clamp(entity.getFadeProgress(), 0, 1);
        if(alpha < 0.98f){
