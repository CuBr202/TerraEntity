...
public abstract class AbstractExistCodecProvider<T> implements DataProvider {
    ...
    public @NotNull CompletableFuture<?> run(@NotNull CachedOutput cachedOutput) {
        return this.lookupProvider.thenCompose(provider -> {
            ...
            this.jsons.forEach(pair -> {
                this.futures.add(DataProvider.saveStable(cachedOutput, provider, getCodec(), pair.getB(), getPath(pair.getA())));
            });
    ...
    protected JsonObject decode(T checkPoint, HolderLookup.Provider provider){
        return parseCodec(getCodec().encodeStart(provider.createSerializationContext(JsonOps.INSTANCE), checkPoint)).getAsJsonObject();
    }
}
