...
public class ChesterSummonItem<T extends Chester> extends SummonItem<T> {
    ...
    public ChesterSummonItem(Properties properties, RegistryObject<EntityType<T>> entityType) {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (player.isShiftKeyDown()) {
            ...
            int maxSize = ChesterTypes.REGISTRY.get().getEntries().size();
            ...
            // 全局指针未在最后
            data.chestType = ++data.chestType % ChesterTypes.REGISTRY.get().getEntries().size();
            ChesterAttachmentPacketS2C.syncChesterOpenType(data.chestType, data.chestTypeAdditional, (ServerPlayer) player);
    ...
    public void appendHoverText(ItemStack stack, Level context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        ...
        if (Minecraft.getInstance().player != null) {
            ...
            if(data.chestType == globalSize - 1 && data.chestTypeAdditional != 0){
                ...
                Map.Entry<ResourceKey<ChesterType>, ChesterType> index =  globalEntry.stream()
                        .toList().get(data.chestType % globalEntry.size());
